...
object CineStreamExtractors {
    ...
    suspend fun invokeVideasy(
        ...
        // Kita senaraikan moviebox (untuk video) dan myflixerzupcloud (untuk subtitle)
        val servers = listOf(
            "moviebox",
            "myflixerzupcloud"
        )
        ...
        servers.safeAmap { server ->
            ...
            try {
                // Letak try-catch kat sini sebagai safety
                val responseReq = app.get(url, headers = headers)
                
                if (responseReq.isSuccessful) {
                    val enc_data = responseReq.text
                    ...
                    if(response.isSuccessful) {
                        ...
                        // 1. Bahagian Video: Hanya tarik dari "moviebox"
                        if (server == "moviebox" && result.has("sources")) {
                            ...
                            for (i in 0 until sourcesArray.length()) {
                                ...
                                callback.invoke(
                                    newExtractorLink(
                                        ...
                                        // Paksa kualiti 1080p jadi priority paling atas
                                        this.quality = when {
                                            quality.contains("1080", ignoreCase = true) -> 1080
                                            quality.contains("auto", ignoreCase = true) -> 0
                        ...
                        // 2. Bahagian Subtitle: Hanya tarik dari "myflixerzupcloud"
                        if (server == "myflixerzupcloud" && result.has("subtitles")) {
                ...
                // Kalau API myflixerzupcloud atau moviebox buat hal, log error je
                e.printStackTrace()
            }
        }
    }
    ...
    suspend fun invokeVidlink(
        ...
        // ==========================================
        // BAHAGIAN 1: TARIK VIDEO DARI VIDLINK
        // ==========================================
        try {
            ...
            val epJson = app.get(epUrl, headers = vidlinkHeaders).text
            val data = globalGson.fromJson(epJson, VidlinkResponse::class.java)
            val m3u8 = data.stream.playlist

            // Tangkap senarai link, filter ikut kualiti yang dibaca oleh M3u8Helper
            M3u8Helper.generateM3u8(
                "Vidlink",
                m3u8,
                "$vidlinkAPI/",
                ...
                when (link.quality) {
                    1080 -> {
                        // Kalau kualiti dia 1080, kita paksa letak nama "1080p"
                        callback.invoke(
                            ExtractorLink(
                                source = "1080p", 
                                name = "1080p",
                    ...
                    720, 480, 360 -> {
                        // Skip terus. Resolusi ni kita taknak, jadi tak payah hantar ke callback (buang)
                    }
                    else -> {
                        // Baki link (contohnya master playlist / unmapped) kita jadikan "Auto"
                        callback.invoke(
                            ExtractorLink(
                                source = "Auto", 
                                name = "Auto",
        ...
        // ==========================================
        // BAHAGIAN 2: TARIK SUBTITLE DARI MYFLIXERZUPCLOUD
        // ==========================================
        if (!title.isNullOrBlank()) {
            try {
                ...
                // Halakan terus ke server myflixerzupcloud
                val subtitleUrl = if (season == null) {
                    "$videasyAPI/myflixerzupcloud/sources-with-title?title=$encTitle&mediaType=movie&year=$year&tmdbId=$tmdbId&imdbId=$imdbId"
                } else {
                    "$videasyAPI/myflixerzupcloud/sources-with-title?title=$encTitle&mediaType=tv&year=$year&tmdbId=$tmdbId&episodeId=$episode&seasonId=$season&imdbId=$imdbId"
                }

                // Guna safety check untuk panggil URL
                val subtitleReq = app.get(subtitleUrl, headers = videasyHeaders)
                
                if (subtitleReq.isSuccessful) {
                    val enc_data_subtitle = subtitleReq.text

                    val jsonBody = mapOf("text" to enc_data_subtitle, "id" to tmdbId)
                    val response = app.post(
                        "$multiDecryptAPI/dec-videasy",
                        json = jsonBody
                    )
                ...
                // Kalau API myflixerzupcloud gagal/error, video Vidlink still akan play
                e.printStackTrace() 
            }
        }
    }
    ...
    suspend fun invokeAnimekai(
        ...
        suspend fun decrypt(text: String): String {
            val jsonBody = mapOf("text" to text)
            val respText = app.post("$multiDecryptAPI/dec-kai", json = jsonBody).text
            return JSONObject(respText).getJSONObject("result").getString("url")
        }

        fun getEpisodeToken(html: String, episode: Int): String? {
            val doc = Jsoup.parse(html)
            return doc.selectFirst("div.eplist ul.range li a[num=\"$episode\"]")?.attr("token")
        }

        fun parseServersFromHtml(html: String): List<Pair<String, String>> {
            val doc = Jsoup.parse(html)
            ...
            for (grp in groups) {
                ...
                for (span in grp.select("span.server")) {
                    val lid = span.attr("data-lid")
                    if (lid.isNotBlank()) servers.add(serverType to lid)
                }
            }
            return servers
        }
        ...
        val searchHtml = JSONObject(searchJson).getJSONObject("result").getString("html")
        val url = Jsoup.parse(searchHtml).selectFirst("a.aitem")?.attr("href") ?: return
        val id = app.get(animekaiAPI + url).document.selectFirst("div.rate-box")?.attr("data-id") ?: return
        
        val listHtml = JSONObject(app.get("$animekaiAPI/ajax/episodes/list?ani_id=$id&_=${encrypt(id)}").text).getString("result")
        val token = getEpisodeToken(listHtml, episode ?: 1) ?: return
        
        val serversResp = app.get("$animekaiAPI/ajax/links/list?token=$token&_=${encrypt(token)}").text
        val serversHtml = JSONObject(serversResp).getString("result")
        val allServers = parseServersFromHtml(serversHtml)

        // SUIS UTAMA
        var globalFoundEnglish = false

        allServers.safeAmap { (type, lid) ->
            if (globalFoundEnglish) return@safeAmap

            val embedResp = app.get("$animekaiAPI/ajax/links/view?id=$lid&_=${encrypt(lid)}").text
            
            if (globalFoundEnglish) return@safeAmap

            val encryptedUrl = try { JSONObject(embedResp).getString("result") } catch(e: Exception) { return@safeAmap }
            val embedUrl = decrypt(encryptedUrl)
            
            val penapisEnglishSahaja: (SubtitleFile) -> Unit = { sub ->
                if (!globalFoundEnglish) {
                    val lang = sub.lang.lowercase()
                    if (lang.contains("english") || lang == "eng" || lang == "en") {
                        globalFoundEnglish = true
            ...
            if (!globalFoundEnglish) {
                MegaUp().getUrl(embedUrl, "Animekai[$type]", penapisEnglishSahaja, {})
            }
        }
    }
    ...
    suspend fun invokeKisskh(
        ...
        val epsId = if (season == null) resDetail.episodes?.first()?.id else resDetail.episodes?.find { it.number == episode }?.id
        if (epsId == null) return

        // ==========================================
        // TAKTIK TURBO: JALANKAN VIDEO DAN SUBTITLE SERENTAK!
        // ==========================================
        coroutineScope {
            
            // EJEN 1: Panggil API Video
            launch {
                try {
                    val epJson = app.get("$multiDecryptAPI/enc-kisskh?text=$epsId&type=vid", referer = kissKhAPI).text
                    val vid_key = JSONObject(epJson).optString("result").takeIf { it.isNotBlank() } ?: return@launch
                    
                    val sourcesResponse = app.get(
                        "$kissKhAPI/api/DramaList/Episode/$epsId.png?err=false&ts=&time=&kkey=$vid_key",
                        referer = kissKhAPI
                    )
                    if (sourcesResponse.code != 200) return@launch
                        ...
                        listOf(source.video, source.thirdParty).safeAmap { link ->
                            ...
                            when {
                                safeLink.contains(".m3u8") || safeLink.contains(".mp4") -> {
                                    ...
                                    val cleanedLink = safeLink.substringBefore("?").takeIf { it.isNotBlank() } ?: return@safeAmap null
            ...
            // EJEN 2: Panggil API Subtitle Serentak (Tapis English & Malay)
            launch {
                try {
                    val subJson = app.get("$multiDecryptAPI/enc-kisskh?text=$epsId&type=sub").text
                    val sub_key = JSONObject(subJson).optString("result").takeIf { it.isNotBlank() } ?: return@launch
                    
                    val subResponse = app.get("$kissKhAPI/api/Sub/$epsId?kkey=$sub_key", referer = kissKhAPI)
                    if (subResponse.code != 200) return@launch

                    tryParseJson<List<KisskhSubtitle>>(subResponse.text)?.forEach { sub ->
                        val langLabel = sub.label ?: return@forEach
                        val langLow = langLabel.lowercase()
                        
                        // PENAPIS: Benarkan 'english', 'en', 'malay', 'ms' sahaja
                        if (langLow.contains("english") || langLow == "en" || langLow.contains("malay") || langLow == "ms") {
                            val finalLang = getLanguage(langLabel) ?: langLabel
                            subtitleCallback.invoke(
                                newSubtitleFile(finalLang, sub.src ?: return@forEach)
                            )
                        }
                    }
                } catch(e: Exception) { e.printStackTrace() }
    ...
    suspend fun invokeKaido(
        ...
        episode: Int? = null,
        season: Int? = null, // Tambah parameter Season
        subtitleCallback: (SubtitleFile) -> Unit,
        callback: (ExtractorLink) -> Unit 
    ) {
        val headers = mapOf(
            "X-Requested-With" to "XMLHttpRequest",
            "Referer" to "https://kaido.to/",
            "User-Agent" to USER_AGENT
        )
        ...
        val id = if (!cleanInput.contains("/")) {
            ...
            val elements = searchDoc.select(".film-detail .film-name a")
            var targetHref = ""
            
            // Periksa satu-satu mana yang tajuknya TEPAT sama
            for (element in elements) {
                if (element.text().equals(searchQuery, ignoreCase = true)) {
                    targetHref = element.attr("href")
                    break
                }
            }
            // Kalau tak ada yang tepat, ambil result pertama
            if (targetHref.isEmpty()) {
                targetHref = elements.firstOrNull()?.attr("href") ?: ""
            }
            
            if (targetHref.isEmpty()) return
            targetHref.substringBefore("?").substringAfterLast("-")
        } else {
            cleanInput.substringAfterLast("-")
        }

        if (id.isEmpty()) return

        // 3. AMBIL SENARAI EPISOD (Sangat Ketat, Halang Isu One Piece)
        val epListJson = app.get("$kaidoAPI/ajax/episode/list/$id", headers = headers).text
        ...
        // 4. AMBIL SENARAI SERVER (Sub/Dub Fallback)
        val serverJson = app.get("$kaidoAPI/ajax/episode/servers?episodeId=$epId", headers = headers).text
        val serverHtml = try { JSONObject(serverJson).getString("html") } catch(e: Exception) { return }
        val serverDoc = Jsoup.parse(serverHtml)
        
        // Cari 'sub' dulu. Kalau kosong, pakai 'dub'
        var servers = serverDoc.select(".server-item[data-type=\"sub\"]")
        if (servers.isEmpty()) {
            servers = serverDoc.select(".server-item[data-type=\"dub\"]")
        }
        ...
        servers.safeAmap { server ->
            ...
            val sourcesJson = app.get("$kaidoAPI/ajax/episode/sources?id=$dataId", headers = headers).text
            val embedLink = try { JSONObject(sourcesJson).getString("link") } catch(e: Exception) { return@safeAmap }
            
            // PENAPIS 1: SUBTITLE ENGLISH SAHAJA
            val penapisEnglish: (SubtitleFile) -> Unit = { sub ->
                if (!globalFoundEnglish) {
                    val lang = sub.lang.lowercase()
                    if (lang.contains("english") || lang == "eng" || lang == "en") {
                        globalFoundEnglish = true
            ...
            // Hantar kedua-dua penapis masuk ke dalam loadCustomExtractor
            loadCustomExtractor("Kaido[$serverType]", embedLink, "https://kaido.to/", penapisEnglish, penapisVideo)
        }
    }
    ...
    suspend fun invokeMoviebox(
        ...
        for (i in 0 until items.length()) {
            ...
            if (matchResult != null) {
                val languageTag = matchResult.groups[1]?.value
                if (languageTag == null) {
                    uniqueIdsWithLang.putIfAbsent(id, "Original")
                    break 
                }
            }
        }
        ...
        // PERUBAHAN DI SINI: Ditukar daripada .forEach kepada for-loop biasa
        for ((subjectId, language) in uniqueIdsWithLang) {
            ...
            val detailObj = try { JSONObject(detailResponseString) } catch (e: Exception) { continue }
            ...
            val sourceObj = try { JSONObject(downloadResponseString) } catch (e: Exception) { continue }
